/**
 * IMAP access layer for Gmail account.
 *
      ...
      try {
        ...
        throw new ImapError(
          `Could not open mailbox "${mailbox}": ${describe(error)}. ` +
            "Call mail_list_mailboxes to see the exact folder paths available.",
        );
      }
      try {
        return await task(client);
      ...
      if (!message || !message.source) {
        throw new ImapError(
          `No message with UID ${uid} in mailbox "${mailbox}". UIDs are per-mailbox — ` +
            "re-run mail_search_messages or mail_list_messages against this mailbox to get a current UID.",
        );
      }
      ...
      try {
        ...
        throw new ImapError(
          `Could not move messages to "${destination}": ${describe(error)}. ` +
            "Call mail_list_mailboxes to confirm the destination folder path exists.",
        );
      }
    });
  }
      ...
      try {
        ...
        throw new ImapError(
          `Could not append the message to "${mailbox}": ${describe(error)}. ` +
            "Call mail_list_mailboxes to confirm the folder path exists.",
        );
      }
    });
  }
...
export function normalizeImapError(error: unknown): Error {
  ...
  if (detail?.authenticationFailed || /AUTHENTICATIONFAILED|Invalid credentials|LOGIN failed/i.test(message)) {
    return new ImapError(
      "IMAP authentication failed. Check MAIL_EMAIL and MAIL_PASSWORD. If the " +
  ...
  if (code === "ENOTFOUND" || code === "EAI_AGAIN") {
    return new ImapError(
      `Could not resolve the IMAP host. Check MAIL_IMAP_HOST (default imap.gmail.com). Underlying error: ${message}`,
    );
  }
