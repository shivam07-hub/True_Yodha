/** SMTP send + MIME composition for Gmail account. */
...
export function normalizeSmtpError(error: unknown): Error {
  ...
  if (code === "EAUTH" || responseCode === 535) {
    return new SmtpError(
      "SMTP authentication failed. Check MAIL_EMAIL and MAIL_PASSWORD. If the " +
        "mailbox has 2FA enabled, use an app password: https://myaccount.google.com/apppasswords " +
  ...
  if (responseCode === 550 || responseCode === 553) {
    return new SmtpError(
      `The server rejected a recipient or the sender address: ${message}. Verify every ` +
        "recipient address, and that MAIL_EMAIL is the mailbox you are authenticating as.",
    );
  }
  if (code === "ENOTFOUND" || code === "EAI_AGAIN") {
    return new SmtpError(
      `Could not resolve the SMTP host. Check MAIL_SMTP_HOST (default smtp.gmail.com). Underlying error: ${message}`,
    );
  }
