...
export function registerManageTools(server: McpServer, ctx: ServerContext): void {
  server.registerTool(
    "mail_set_flags",
    {
      title: "Set Gmail Message Flags",
      description: `Add or remove IMAP flags on one or more Gmail messages.
...
Examples:
  ...
  - Don't use when: You want the message out of the inbox (use mail_move_messages)
...
Error Handling:
  ...
  server.registerTool(
    "mail_move_messages",
    {
      title: "Move Gmail Messages",
      description: `Move messages from one Gmail folder to another.
...
Examples:
  ...
  - Don't use when: You want to delete mail (use mail_delete_messages)

Error Handling:
  - An unknown destination returns an error naming it; call mail_list_mailboxes
  ...
  server.registerTool(
    ...
    {
      title: "Delete Gmail Messages",
      description: `Delete Gmail messages, by default by moving them to Trash (recoverable).
...
Examples:
  ...
  - Don't use when: The user just wants mail out of the inbox (use mail_move_messages)
  ...
  - Returns an error if no Trash folder can be found; pass an explicit destination
    ...
    async ({ mailbox, uids, permanent, confirm }) => {
      try {
        ...
        if (!trash) {
          throw new ImapError(
            "No folder is marked as Trash on this account. Call mail_list_mailboxes " +
              "and use mail_move_messages with an explicit destination instead.",
          );
        }
        if (trash === mailbox) {
          throw new ImapError(
