...
```bash
cd ~/True_Yodha/mcp-servers/gmail-email-mcp-server
npm install
npm run build
...
```bash
claude mcp add gmail-himirko -- /Users/incognito/True_Yodha/mcp-servers/gmail-email-mcp-server/scripts/run.sh
```
...
```json
{
  "mcpServers": {
    "gmail-himirko": {
      "command": "/Users/incognito/True_Yodha/mcp-servers/gmail-email-mcp-server/scripts/run.sh"
    }
  }
}
```
