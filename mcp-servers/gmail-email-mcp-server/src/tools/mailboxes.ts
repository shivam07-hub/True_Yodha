...
export function registerMailboxTools(server: McpServer, ctx: ServerContext): void {
  server.registerTool(
    "mail_list_mailboxes",
    {
      ...
      description: `List every IMAP folder in the connected Gmail mailbox.

Call this first whenever a folder path is needed: every other tool takes a
case-sensitive folder path, and Gmail account layouts differ (some use "Sent",
...
Examples:
  ...
  - Don't use when: You need message counts (use mail_mailbox_status instead)`,
  ...
  server.registerTool(
    "mail_mailbox_status",
    {
      title: "Get Gmail Mailbox Status",
      description: `Get message counts for one Gmail folder: total, unread, and recent.
...
Examples:
  ...
  - Don't use when: You need the actual messages (use mail_list_messages)

Error Handling:
  - Returns an error naming the folder if the path does not exist; call
    mail_list_mailboxes for exact paths.`,
  ...
  server.registerTool(
    ...
    {
      title: "Check Gmail Connection",
      description: `Verify that the configured Gmail credentials work for IMAP and SMTP.
...
Returns:
  {
    ...
    "sending_enabled": boolean, // false unless MAIL_ALLOW_SEND=true
...
Examples:
  ...
  - Don't use when: You simply need folder names (use mail_list_mailboxes)`,
      inputSchema: {
        check_smtp: z
          .boolean()
          .default(true)
    ...
    async ({ check_smtp }) => {
      ...
      const lines = [
        `# Gmail connection: ${ctx.config.email}`,
        "",
        ...
        `- **Sending enabled**: ${ctx.config.allowSend ? "yes" : "no (set MAIL_ALLOW_SEND=true to enable mail_send_email)"}`,
      ];
      return toolResult(lines.join("\n"), output);
    },
  );
