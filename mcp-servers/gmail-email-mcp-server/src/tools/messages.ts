  ...
  if (messages.length === 0) {
    return toolResult(
      `No messages matched in "${params.mailbox}". Widen the filters, or call ` +
        "mail_list_mailboxes to confirm the folder path.",
      output,
    );
  }
  const header =
...
export function registerMessageTools(server: McpServer, ctx: ServerContext): void {
  server.registerTool(
    "mail_list_messages",
    {
      ...
      description: `List messages in one Gmail folder, newest first.

Returns envelope data only (sender, subject, date, flags) — never message
bodies. Use mail_get_message with a UID from this list to read one message.
...
Returns:
  For JSON format:
  {
    ...
    "messages": [
      {
        "uid": number,            // Pass to mail_get_message; unique within this mailbox only
        "mailbox": string,
...
Examples:
  ...
  - Don't use when: You are filtering by sender, subject, or date (use mail_search_messages)`,
      inputSchema: {
  ...
  server.registerTool(
    "mail_search_messages",
    {
      ...
      description: `Search one Gmail folder by sender, recipient, subject, free text, date, or state.
...
and contain envelope data only — use mail_get_message to read a body.
...
Returns:
  Same shape as mail_list_messages.
...
Examples:
  ...
  - Don't use when: You want the newest mail with no filter (use mail_list_messages)

Error Handling:
  - Searching an unknown folder returns an error naming it; call mail_list_mailboxes
    for exact paths.
  ...
  server.registerTool(
    "mail_get_message",
    {
      ...
      description: `Read one Gmail message in full: headers, body text, and attachment metadata.
...
mail_download_attachment to fetch one.

Args:
  - mailbox (string): Folder holding the message (default: "INBOX")
  - uid (number): Message UID from mail_list_messages or mail_search_messages (required)
...
Examples:
  - Use when: "Read the email from Stripe" -> first mail_search_messages, then this with its uid
  - Use when: You need to know what a message's attachments are before downloading one
  - Don't use when: You only need subjects and senders (use mail_list_messages)
...
Error Handling:
  ...
  server.registerTool(
    "mail_download_attachment",
    {
      ...
      description: `Download one attachment from a Gmail message to a local file.

Identify the attachment by the 'index' shown in mail_get_message. The file is
written to the server's attachment directory (MAIL_ATTACHMENT_DIR, default a
...
Args:
  ...
  - attachment_index (number): Zero-based index from mail_get_message (required)
...
Examples:
  - Use when: "Save the PDF from that invoice email" -> mail_get_message first, then attachment_index=0
  - Use when: You need the file on disk to process it with another tool
  - Don't use when: You just want to know what is attached (mail_get_message already lists that)
  ...
  - Returns an error if attachment_index is out of range, listing how many attachments exist.`,
      inputSchema: {
        ...
        attachment_index: z
          ...
          .int()
          .min(0)
          .describe("Zero-based attachment index as reported by mail_get_message."),
        include_base64: z
          .boolean()
          .default(false)
        ...
        if (!attachment) {
          throw new ImapError(
            `Attachment index ${attachment_index} does not exist on UID ${uid}; the ` +
              `message has ${attachments.length} attachment(s). Call mail_get_message ` +
