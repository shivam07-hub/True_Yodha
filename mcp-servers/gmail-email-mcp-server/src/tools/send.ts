...
export function registerSendTools(server: McpServer, ctx: ServerContext): void {
  server.registerTool(
    "mail_send_email",
    {
      ...
      description: `Send an email from the connected Gmail mailbox via SMTP.
...
also disabled at the server level unless MAIL_ALLOW_SEND=true.
...
Examples:
  ...
  - Use when: Replying in-thread -> pass in_reply_to and references from mail_get_message
  - Don't use when: The user has not approved the exact text (use mail_save_draft instead)

Error Handling:
  - Returns an error if MAIL_ALLOW_SEND is not "true", naming the env var to set.
    ...
    async (params) => {
      try {
        if (!ctx.config.allowSend) {
          throw new SmtpError(
            "Sending is disabled on this MCP server. Set MAIL_ALLOW_SEND=true in the " +
              "server environment and restart it to allow outbound mail. Until then, use " +
              "mail_save_draft to stage the message in the Drafts folder.",
          );
        }
  ...
  server.registerTool(
    "mail_save_draft",
    {
      ...
      description: `Compose a message and save it to the Gmail Drafts folder without sending it.

The safe way to prepare mail: the user can review and send it from webmail, or
approve it and then call mail_send_email. Nothing leaves the account.
...
Examples:
  ...
  - Use when: Sending is disabled (MAIL_ALLOW_SEND is not true) but the user wants the text staged
  - Don't use when: The user has already approved sending (use mail_send_email)

Error Handling:
  - Returns an error if no Drafts folder can be found; pass 'mailbox' explicitly after
    checking mail_list_mailboxes.`,
      inputSchema: {
    ...
    async (params) => {
      try {
        ...
        if (!drafts) {
          throw new SmtpError(
            "No folder is marked as Drafts on this account. Call mail_list_mailboxes " +
