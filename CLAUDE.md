...
25. **Rename "XP" / "Tokens" → "Myro Coins" — COPY PASS DONE, identifier/DB pass OPEN.** ✅ Public/logged-out copy (2026-06-13) + ✅ **authed visible-copy pass committed + pushed to Develop 2026-06-16 (`f5e30d0`)** — every rendered "XP"/"Tokens"/"coins" string → "Myro Coins" across home/diary/cv/market/upskilling/settings/onboarding/companies/newsletter; tsc 0 · lint clean · brand-system 6/6. **STILL OPEN — step 2 (SEPARATE PR, do NOT mix with copy):** identifier/contract rename — `backend/app/services/xp_service.py`, `frontend/lib/xp-policy.ts`→`myro-coins-policy.ts`, identifiers `XP_POLICY`/`useXPGate`/`TOKEN_TIERS`/`types/xp`, the `frontend/components/xp/` folder, the `frontend/app/(authed)/tokens/` route (keep alive via redirect), API request/response field names, and DB columns/keys (`daily_logs.skills_delta` action keys + any `xp`-named cols/RPC params). Each renamed API/column needs a migration + back-compat read shim (accept old+new for one release, then drop old); fresh-grep before starting (counts drift). Counsel-gated Terms §07 + privacy "tokens" stay deferred to #17. Dev code-comments still say "tokens" (cosmetic — fold into step 2). Rationale: "Token" collides with auth/LLM tokens, "XP" over-gamifies; canonical = **Myro Coins**.
