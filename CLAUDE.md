...
21. **Read-path latency at scale — bootstrap PARALLELIZED 2026-06-13, rest OPEN.** Prod logs showed pervasive `metric route.slow`: `/home/bootstrap` 4–6.5s, `/jobs/feed` 5–6s, `/jobs/my-skills/demand` 5–6.7s, `/jobs/analytics/me` 3.8–4.5s, plus the **Intel skill-heatmap thundering herd** — one `useQuery` PER followed company (IH3 by design), so a heatmap fires 10–15 parallel `/jobs/analytics/skill-heatmap?companies=X&skills=<~40 skills>` requests at once; several hit 1.2–3.6s and one died mid-stream with `httpcore.WriteError: [Errno 32] Broken pipe` → 500.
   ...
   **STILL OPEN (separate pages, not the login path — pick up when working those surfaces):** (a) `/jobs/feed` 5–6s + `/jobs/my-skills/demand` 5–6.7s — profile these the same way (serial fan-out? N+1? missing index?). The feed is NOT a multi-section BFF, so its 5s is likely a genuine slow query/RPC — needs EXPLAIN ANALYZE, not just parallelization. (b) **Intel heatmap fan-out** — IH3's per-company `useQuery` is 10–15 parallel requests; decide ONE batched `?companies=a,b,c` round-trip vs keeping the fan-out (IH3 chose fan-out for incremental row-append; batching trades that for fewer connections — re-grill IH3 if changing). (c) **broken-pipe 500** = client aborted (heatmap row unmounted / navigated) while server still streamed → needs graceful client-abort handling (treat `WriteError`/`BrokenPipe` as a client-disconnect, not a 500), independent of the latency work.
