...
25. **Rename "XP" / "Tokens" → "Myro Coins" — COPY DONE; step-2 PLAN LOCKED, PR1 pushed (apply-gated).** ✅ Public copy (2026-06-13) + ✅ authed visible-copy pass (Develop `f5e30d0`, 2026-06-16). **Step-2 grilled + locked 2026-06-16 (Shivam = full purity):** rename the internal `xp`→`coins` across DB + wire + identifiers via **expand/contract**. Names: `xp_balance`→`coin_balance`, `welcome/linkedin_xp_granted`→`*_coins_granted`, `xp_ledger`→`coin_ledger`, RPCs `charge_xp/refund_xp/reward_xp`→`charge_coins/...`, wire `xp_earned`→`coins_earned`/`new_xp_balance`→`new_coin_balance`, frontend `XP_POLICY`→`MYRO_COINS_POLICY`/`useXPGate`→`useCoinsGate`/`lib/xp-policy.ts`→`coins-policy.ts`/`components/xp/`→`components/coins/`/`types/xp`. **Decisions:** extension is NOT a consumer (only `/jobs/import`) so wire rename only risks frontend↔backend deploy skew → **dual-emit SKIPPED, wire renamed in lockstep in PR2** (only cost = cosmetic self-healing blip on the transient "+earned" number in a ~30s window; balances stay correct via the passthrough column). `daily_logs.skills_delta` keys are semantic action names (no literal "xp") — nothing to rename there.
    ...
    - **PR2 CUTOVER (next, after PR1 applied):** backend `xp_service` calls coin wrappers + reads `coin_balance`; rename wire fields (lockstep) + the ~10 response models; frontend renames identifiers (~19 files) + reads `coin_balance`/`coins_earned`; `/tokens` route kept via redirect; update `xp_service.py`→`coin_service.py`; tests. Frontend + backend ship in ONE Develop push (lockstep).
    - **PR3 CONTRACT (weeks later, after soak):** drop `xp_balance`/`xp_ledger`/old RPCs/old wire fields once nothing reads them. Separate migration, Shivam-applied.
