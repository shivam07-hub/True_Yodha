...
32. **Design-system standardization + button cascade — IN PROGRESS (2026-06-21/22, all on Develop, pushed).** Trigger: Shivam — *"one website / consistency = trust"*; caught the nav rendering as two parallel CSS systems + a logged-in user seeing an anonymous CTA. Principle: standardize the shared concept via a **single source both consumers read**, enforced by a build gate, never by discipline. Full system + file map in memory `project_truyodha_standardization_system`.
    ...
    **REMAINING — the button cascade (~80 inline `<button style>` files left, surface-by-surface; each: full `tsc` + lint + `check:ui-drift` + Shivam visual QC, then push).** Biggest remaining surfaces: `public-playground (16) · settings-modal (14) · playground-view (14) · quiz-runner (10) · cv-export-view (7)` + CV builder views (baseline/library/playground) + `skills/*` + `onboarding/*` + diary + market/dashboard cards (mixed buttons + chips — careful). **Rules:** migrate genuine CTA/action buttons only → `solid`/`neutral`/`danger`/`ghost`; use `CloseButton` for ✕. **LEAVE** selectors/chips (filter pills, stage chips, severity selectors, star ratings) — a separate future "chip" standardization — and **intentionally-different** buttons (`new-report.tsx` submit is category-colored bug=red/praise=green, not drift). When a button has no exact variant, pick the closest; don't invent. The guard keeps NEW drift out while the sweep lowers the backlog count. Memory: `project_truyodha_standardization_system`, `feedback_standardization_trust`.
