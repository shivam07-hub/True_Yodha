...
## LAST SESSION SUMMARY (2026-06-16 · Backlog #25 authed copy pass — committed + pushed)

Closed the authed-app half of #25 (Myro Coins rename). Fresh grep found most named heavy files (home/forge/market/settings/`xp-policy.ts`) **already migrated** — backlog claim was stale (verify-in-code rule). Fixed only the true stragglers: rendered "tokens"/"coins" → "Myro Coins" in `loop-bar`, `shipped`, `intel-drawer`, upskilling `results`/`upskilling-home`/`upskilling-view`/`gap-readiness`, `DiaryPanel`, `MatchRefreshGate`, `settings-modal` (+ the pre-migrated cv/market/home/companies/onboarding/newsletter files staged in the same commit). **Left by scope (step-2 PR):** identifiers (`XP_POLICY`/`useXPGate`/`TOKEN_TIERS`/`types/xp`/`components/xp/`), `/tokens` route, dev comments; **left legal:** Terms/privacy "tokens" (#17). 23 files, **committed + pushed Develop `f5e30d0`** (real deploy repo = outer `shivam07-hub/True_Yodha`; the nested `frontend/.git` is a remote-less mid-reshuffle artifact — ignore for push). tsc 0 · lint clean · brand-system 6/6. Also trimmed this CLAUDE.md (Sprint 4/5/6 + BETA-2 plans folded to a consolidated open-items pointer).
