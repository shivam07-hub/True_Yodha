...
| Optional Turnstile hardening | Missing keys repeatedly crashed Railway/Vercel even though Turnstile is non-critical at the current stage | **POLICY UPDATED** | Turnstile now has explicit off-by-default frontend/backend flags. While disabled, existing per-IP limits remain active. If enabled without its keys, only protected anonymous routes fail closed; API startup and frontend builds remain available. |
| Critical frontend values stop production build/start | Not enforced centrally | **FAIL → FIXED** | `prebuild` and `prestart` validate Supabase URL/key, API URL, and Razorpay public key. Optional internal API, canonical site URL, and Turnstile values use runtime fallbacks or feature-scoped handling. Configured service URLs must use HTTPS. |
