...
- **`ALLOWED_ORIGINS` is enforced.** Production startup rejects missing, wildcard, malformed, or non-HTTPS origins. Set the exact Vercel domains for each backend service; browser preflights from every other origin are rejected.
