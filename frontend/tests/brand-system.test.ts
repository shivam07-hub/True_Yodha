...
test("layout follows the OS surface by default and uses the platform stack", () => {
  ...
  assert.match(tokens, /--tm-font-sans:\s*-apple-system/)
  assert.doesNotMatch(tokens, /--tm-font-sans-phone/)
  assert.match(tokens, /font-size:\s*100%/)
  ...
  assert.match(tokens, /\.tm-feed-page \.mc-workspace/)
  const landing = read("components/public/landing/landing-base.css")
  assert.match(landing, /--lp-fs-hero:\s*var\(--tm-fs-hero\)/)
  assert.match(landing, /--lp-fs-body:\s*var\(--tm-fs-body\)/)
  assert.doesNotMatch(read("components/public/landing-page.tsx"), /fontClassName/)
