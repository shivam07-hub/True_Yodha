    ...
    <AppShell>
      <div style={{ padding: "28px 32px", overflowY: "auto", height: "100%" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 16, alignItems: "start" }}>
          ...
          {/* Skill velocity sidebar */}
          <div style={{ position: "sticky", top: 0, display: "flex", flexDirection: "column", gap: 12 }}>
            {/* Gap skills */}
            <div style={{ background: "rgba(123,47,255,0.06)", border: "1px solid rgba(123,47,255,0.2)", borderRadius: 14, padding: 20 }}>
              <div style={{ fontSize: 10, letterSpacing: "0.12em", textTransform: "uppercase", color: "rgba(123,47,255,0.8)", marginBottom: 14 }}>
                Skill Velocity
              </div>
              {topGapSkills.length > 0 ? (
                ...
                <div style={{ fontSize: 12, color: "rgba(240,244,255,0.4)", padding: "16px 0" }}>
                  Compute your score to see skill gaps.
                </div>
              )}
            </div>

            {/* Tracked applications summary */}
            {appsQuery.data && appsQuery.data.length > 0 && (
              <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(0,245,212,0.1)", borderRadius: 14, padding: 20 }}>
                ...
                {appsQuery.data.slice(0, 4).map((app) => (
                  <div key={app.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                    <div>
                      <div style={{ fontSize: 12, color: "#F0F4FF", fontWeight: 500 }}>{app.title}</div>
                      <div style={{ fontSize: 10, color: "rgba(240,244,255,0.4)" }}>{app.company}</div>
                    </div>
                    <span style={{
                      fontSize: 10, padding: "2px 8px", borderRadius: 999,
                      background: `${STATUS_META[app.status].color}18`,
