    ...
    <>
      ...
      <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", zIndex: 61, width: "min(480px,92vw)", background: "var(--tm-surface)", border: "1px solid var(--tm-int-border)", borderRadius: 14, padding: 28, display: "flex", flexDirection: "column", gap: 20, boxShadow: "0 24px 80px rgba(0,0,0,0.6), 0 0 40px var(--tm-int-bg-wash)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <div style={{ fontFamily: "var(--tm-font-mono)", fontSize: "var(--tm-fs-caption)", letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--tm-interactive)", marginBottom: 4 }}>Leave a review</div>
            <div style={{ fontSize: "var(--tm-fs-heading)", fontWeight: 600, color: "var(--tm-text)" }}>{company ?? "Company"}</div>
          </div>
          <CloseButton onClick={onClose} />
        </div>

        <div>
          <div style={{ fontSize: "var(--tm-fs-caption)", color: "var(--tm-text-faint)", marginBottom: 8 }}>Overall experience</div>
          <div style={{ display: "flex", gap: 6 }} onMouseLeave={() => setHoveredStar(0)}>
            {[1, 2, 3, 4, 5].map(n => (
              <button
                ...
                onClick={() => setStars(n)}
                style={{ fontSize: "var(--tm-fs-title)", background: "none", border: "none", cursor: "pointer", padding: 0, lineHeight: 1, color: n <= displayStar ? "#FBBF24" : "var(--tm-border)", transition: "color 80ms ease" }}
              >
                ★
              </button>
            ))}
        ...
        <div>
          <div style={{ fontSize: "var(--tm-fs-caption)", color: "var(--tm-text-faint)", marginBottom: 8 }}>Note <span style={{ opacity: 0.5 }}>(optional)</span></div>
          <textarea
            value={note}
            onChange={e => setNote(e.target.value)}
            rows={3}
            placeholder="What happened? Anything others should know..."
            style={{ width: "100%", background: "rgba(255,255,255,0.03)", border: "1px solid var(--tm-border)", borderRadius: 8, padding: "10px 12px", fontSize: "var(--tm-fs-body)", color: "var(--tm-text)", fontFamily: "inherit", resize: "vertical", outline: "none", boxSizing: "border-box" }}
          />
        </div>
