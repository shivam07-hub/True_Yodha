 ...
 *
 * The map for the weave: every JD requirement classified against the user's
 * career stories AND the CV's own lines — covered / partial / missing. The
 * primary action (and any row) opens Tailor with Mentor. Replaces the old
...
export function CoveragePanel({ coverage, loading, error, onOpenWeave, onRetry }: CoveragePanelProps) {
  if (loading) {
    return (
  ...
  return (
    <div className="cvb-cov">
      ...
      <button type="button" className="tw-btn tw-btn-primary cvb-cov-cta" onClick={onOpenWeave}>
        Tailor with Mentor →
      </button>
      <ul className="cvb-cov-list">
        {coverage.requirements.map((r, i) => (
          <li key={i}>
            <button type="button" className="cvb-cov-row" data-v={r.status} onClick={onOpenWeave}>
