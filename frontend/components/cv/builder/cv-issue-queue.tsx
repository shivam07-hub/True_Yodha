/**
 * CvIssueQueue — rank 3. One row per open issue, and nothing else at rest.
 *
 * `3px 1fr auto`: severity gutter · title (12.5/600) + provenance (10 mono
 * uppercase) · chevron. The row carries no description paragraph, no `bullet →`
 * quote of the line it points at, and no inline rewrite — all three were in the
 * old fix card, all three duplicated something the CV pane already shows, and
 * together they made a five-issue rail scroll past two screens.
 *
 * Expanding a row opens the BRIEF: what this costs, why, and an authored example
 * of the same defect fixed. Free, instant, no network. That is the difference
 * between this and the version shipped on 25 Aug, where the only way to learn
 * what was wrong with a line was to start a Mentor call and wait — expanding a
 * row and generating a rewrite were the same click.
 *
 * ATS rows live in this same list (§4.4) and are actionable on every surface,
 * including logged out — the old anon ATS tab rendered a red ✗ the user could
 * not do anything about — and they carry a brief too, for the same reason.
 */
"use client"

import { CvFixBrief } from "./cv-fix-brief"
import type { Issue } from "./issue-model"

interface CvIssueQueueProps {
  issues: Issue[]
  /** Id of the issue whose line is currently open in the CV pane. */
  activeId: string | null
  /** Id of the row expanded to its brief. Expanding costs nothing. */
  openId: string | null
  onToggle: (issue: Issue) => void
  /** Take me to the line and open the fix there. */
  onGo: (issue: Issue) => void
  onDismiss?: (issue: Issue) => void
  emptyNote?: string
}

export function CvIssueQueue({
  issues, activeId, openId, onToggle, onGo, onDismiss, emptyNote,
}: CvIssueQueueProps) {
  if (issues.length === 0) {
    return emptyNote ? <p className="cvw-queue cvw-issue-prov">{emptyNote}</p> : null
  }
  return (
    <div className="cvw-queue">
      {issues.map(issue => {
        const active = issue.id === activeId
        const open = issue.id === openId
        return (
          <div key={issue.id} className="cvw-issue-wrap">
            <button
              type="button"
              className="cvw-issue"
              data-sev={issue.severity}
              data-active={active}
              aria-expanded={open}
              onClick={() => onToggle(issue)}
            >
              <span className="cvw-gutter" aria-hidden />
              <span className="cvw-issue-body">
                <span className="cvw-issue-title">{issue.title}</span>
                {/* The active row swaps provenance for the handoff: the line is
                    already open over in the pane, so where it lives is no longer
                    the useful fact — where to look is. */}
                {active
                  ? <span className="cvw-issue-prov cvw-issue-open">open on the line →</span>
                  : issue.provenance
                    ? <span className="cvw-issue-prov">{issue.provenance}</span>
                    : null}
              </span>
              {active
                ? <span className="cvw-issue-dot" aria-hidden />
                : <span className="cvw-issue-act">{open ? "▴" : issue.action ?? "›"}</span>}
            </button>

            {open && (
              <CvFixBrief
                brief={issue.brief}
                severity={issue.severity}
                onJump={() => onGo(issue)}
                onDismiss={onDismiss && issue.fix ? () => onDismiss(issue) : undefined}
              />
            )}
          </div>
        )
      })}
    </div>
  )
}
