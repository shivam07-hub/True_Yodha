    ...
    return (
      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
        <span style={{
          ...
          color: "var(--tm-accent)", fontSize: 12, fontWeight: 700,
        }}>
          ✓ Done
        </span>
      </div>
  ...
  return (
    <div style={{
      ...
      <textarea
        ...
        onFocus={(e) => { e.currentTarget.style.borderColor = "var(--tm-accent-ring)" }}
        onBlur={(e) => { e.currentTarget.style.borderColor = "var(--tm-border)" }}
      />
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span style={{ fontSize: 11, color: "var(--tm-text-faint)" }}>Save proof to claim XP</span>
        <button
          onClick={onSave}
          disabled={!proofText.trim() || saving}
          style={{
            padding: "7px 14px", borderRadius: 99,
            background: proofText.trim() ? "var(--tm-accent)" : "rgba(255,255,255,0.04)",
            border: "none",
            color: proofText.trim() ? "var(--tm-accent-fg)" : "var(--tm-text-faint)",
