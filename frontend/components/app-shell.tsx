    ...
    >
      ...
      >
        ...
        <div style={{ padding: "32px 32px 24px", borderBottom: "1px solid var(--tm-border-soft)", textAlign: "center", position: "relative" }}>
          <button
            onClick={onClose}
            style={{ position: "absolute", top: 20, right: 20, background: "transparent", border: "none", color: "var(--tm-text-faint)", fontSize: 22, cursor: "pointer" }}
          >×</button>
          <div style={{
          ...
          <p style={{ fontSize: 15, color: "var(--tm-text-muted)", fontStyle: "italic" }}>
            See yourself clearly. For the first time.
          </p>
          <p style={{ fontSize: 12, color: "var(--tm-text-faint)", marginTop: 10, lineHeight: 1.6, maxWidth: 420, margin: "10px auto 0" }}>
        ...
        <div style={{ padding: "24px 32px", display: "flex", flexDirection: "column", gap: 28 }}>
          ...
          <div style={{ padding: "18px 20px", borderRadius: "var(--tm-radius)", background: "var(--tm-warning-wash)", border: "1px solid var(--tm-border)" }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 600, color: "var(--tm-text)", marginBottom: 12 }}>
            ...
            {[
              ...
              <div key={i} style={{ display: "flex", gap: 8, alignItems: "flex-start", marginBottom: 6 }}>
                ...
                <span style={{ fontSize: 12, color: "var(--tm-text-muted)", lineHeight: 1.6 }}>{t}</span>
              </div>
            ))}
          </div>
          ...
          <div>
            ...
            <blockquote style={{ borderLeft: "3px solid var(--tm-accent)", paddingLeft: 16, margin: 0 }}>
              <p style={{ fontSize: 14, fontWeight: 500, color: "var(--tm-text)", lineHeight: 1.7, marginBottom: 8 }}>
              ...
              <p style={{ fontSize: 12, color: "var(--tm-text-faint)", lineHeight: 1.6 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {howItWorks.map((s) => (
                <div key={s.n} style={{ display: "flex", gap: 14, padding: "12px 16px", borderRadius: "var(--tm-radius)", background: "rgba(255,255,255,0.02)", border: "1px solid var(--tm-border-soft)" }}>
                  <div style={{ fontSize: 10, fontWeight: 700, color: "var(--tm-text-faint)", minWidth: 20, paddingTop: 1 }}>{s.n}</div>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 500, color: "var(--tm-text)", marginBottom: 2 }}>{s.title}</div>
                    <div style={{ fontSize: 11, color: "var(--tm-text-faint)", lineHeight: 1.5 }}>{s.body}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ padding: "18px 20px", borderRadius: "var(--tm-radius)", background: "var(--tm-surface-2)", border: "1px solid var(--tm-border)" }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 600, color: "var(--tm-text)", lineHeight: 1.4, marginBottom: 12 }}>
            ...
            <div style={{ padding: "14px 16px", borderRadius: "var(--tm-radius-sm)", background: "rgba(255,255,255,0.03)", border: "1px solid var(--tm-border-soft)", fontSize: 12, color: "var(--tm-text-muted)", lineHeight: 1.6, fontStyle: "italic" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {values.map((v) => (
                <div key={v.title} style={{ display: "flex", gap: 14, padding: "12px 16px", borderRadius: "var(--tm-radius)", background: "rgba(255,255,255,0.02)", border: "1px solid var(--tm-border-soft)" }}>
                  <div style={{ fontSize: 16, color: "var(--tm-accent)", minWidth: 22, paddingTop: 1, filter: "drop-shadow(0 0 4px var(--tm-accent-glow))" }}>{v.icon}</div>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 500, color: "var(--tm-text)", marginBottom: 2 }}>{v.title}</div>
                    <div style={{ fontSize: 11, color: "var(--tm-text-faint)", lineHeight: 1.5 }}>{v.body}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ textAlign: "center", padding: "20px", borderRadius: "var(--tm-radius)", background: "var(--tm-accent-wash)", border: "1px solid var(--tm-accent-ring)" }}>
            <div style={{ fontSize: 18, fontWeight: 700, color: "var(--tm-text)", marginBottom: 6 }}>Look in the mirror.</div>
            <div style={{ fontSize: 12, color: "var(--tm-text-muted)", marginBottom: 14 }}>See where you really stand. Then do something about it.</div>
            <button
              onClick={onClose}
      ...
      >
        {sent ? (
          <div style={{ textAlign: "center", padding: "16px 0" }}>
            ...
            <div style={{ fontSize: 13, fontWeight: 500, color: "var(--tm-text)" }}>Thanks — received!</div>
          </div>
        ) : (
          <>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
              <span style={{ fontSize: 16, color: action.color, filter: `drop-shadow(0 0 4px ${action.color})` }}>{action.icon}</span>
              <span style={{ fontSize: 13, fontWeight: 600, color: "var(--tm-text)" }}>{action.label}</span>
              <button
                onClick={onClose}
                style={{ marginLeft: "auto", background: "transparent", border: "none", color: "var(--tm-text-faint)", fontSize: 18, cursor: "pointer", lineHeight: 1 }}
              >×</button>
            </div>
            <textarea
              value={text}
              ...
              style={{
                ...
                border: `1px solid ${action.bg}`,
                color: "var(--tm-text)", fontSize: 12, lineHeight: 1.6,
            ...
            <button
              ...
              style={{
                ...
                color: action.color, fontSize: 12, fontWeight: 500,
        ...
        {expanded && menuOpen && (
          <div style={{ padding: "8px 8px 0", display: "flex", flexDirection: "column", gap: 2 }}>
            {FEEDBACK_ACTIONS.map((a) => (
              ...
              >
                <span style={{ fontSize: 13, color: a.color, minWidth: 18, textAlign: "center", filter: `drop-shadow(0 0 3px ${a.color})` }}>{a.icon}</span>
                <span style={{ fontSize: 12, color: "var(--tm-text-muted)" }}>{a.label}</span>
              </button>
            ))}
      ...
      {signOutConfirm && (
        ...
        >
          ...
          <div
            ...
            onClick={(e) => e.stopPropagation()}
          >
            <div style={{ fontSize: 28, marginBottom: 12, color: "var(--tm-text-muted)" }}>→</div>
            <div style={{ fontSize: 15, fontWeight: 600, color: "var(--tm-text)", marginBottom: 6 }}>Sign out?</div>
            <div style={{ fontSize: 12, color: "var(--tm-text-muted)", marginBottom: 24, lineHeight: 1.6 }}>
              Your progress is saved. You can sign back in anytime to resume your journey.
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <button
                onClick={() => setSignOutConfirm(false)}
                style={{ flex: 1, padding: "10px", borderRadius: "var(--tm-radius)", background: "rgba(255,255,255,0.04)", border: "1px solid var(--tm-border)", color: "var(--tm-text-muted)", fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}
              >
                Cancel
              </button>
              <button
                onClick={signOut}
                style={{ flex: 1, padding: "10px", borderRadius: "var(--tm-radius)", background: "rgba(255,80,80,0.1)", border: "1px solid rgba(255,80,80,0.25)", color: "rgba(255,130,130,0.9)", fontSize: 12, fontWeight: 500, cursor: "pointer", fontFamily: "inherit" }}
              >
                Sign out
              </button>
            </div>
    ...
    >
      ...
      >
        ...
        <div style={{ opacity: expanded ? 1 : 0, transition: `opacity var(--tm-dur)`, whiteSpace: "nowrap", overflow: "hidden" }}>
          ...
          <div className="tm-label-caps" style={{ marginTop: 2, fontSize: 9 }}>Career Intelligence</div>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ opacity: expanded ? 1 : 0, transition: `opacity var(--tm-dur)`, whiteSpace: "nowrap" }}>
          <div className="tm-label-caps" style={{ fontSize: 9 }}>Truth Score</div>
          <div style={{ fontSize: 11, color: "var(--tm-text-muted)", marginTop: 2 }}>Market position</div>
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              <div style={{ opacity: expanded ? 1 : 0, transition: `opacity var(--tm-dur)`, overflow: "hidden", whiteSpace: "nowrap" }}>
                <div style={{ fontSize: 13, fontWeight: 500, color: active ? "var(--tm-accent)" : "var(--tm-text)" }}>
                  {item.label}
                </div>
      ...
      {expanded && (
        <div style={{
          ...
          <div className="tm-label-caps" style={{ fontSize: 9 }}>Accent</div>
          <AccentToggle />
        </div>
      )}
