-- A verifier claim is a queue lease, not product evidence. Keep its hot clock
-- in a narrow table so claiming 200 jobs does not rewrite the wide, heavily
-- indexed jobs row before the verifier writes the actual verdict.

CREATE TABLE IF NOT EXISTS public.job_verification_schedule (
    job_id text PRIMARY KEY REFERENCES public.jobs(job_id) ON DELETE CASCADE,
    last_attempt_at timestamptz
);

COMMENT ON TABLE public.job_verification_schedule IS
    'Narrow lease/schedule read model for listing verification. Product evidence remains on jobs; claim churn does not rewrite jobs.';

ALTER TABLE public.job_verification_schedule ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON TABLE public.job_verification_schedule FROM PUBLIC, anon, authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.job_verification_schedule TO service_role;

CREATE INDEX IF NOT EXISTS idx_job_verification_schedule_due
    ON public.job_verification_schedule (last_attempt_at ASC NULLS FIRST, job_id);

INSERT INTO public.job_verification_schedule (job_id, last_attempt_at)
SELECT j.job_id, j.last_verification_attempt_at
FROM public.jobs j
...
BEGIN
    IF NEW.retired_at IS NULL AND NEW.apply_url LIKE 'http%' THEN
        INSERT INTO public.job_verification_schedule (job_id, last_attempt_at)
        VALUES (NEW.job_id, NEW.last_verification_attempt_at)
        ON CONFLICT (job_id) DO UPDATE SET
            last_attempt_at = EXCLUDED.last_attempt_at;
    ELSE
...
REVOKE ALL ON FUNCTION public.sync_job_verification_schedule()
...
AFTER INSERT OR UPDATE OF last_verification_attempt_at, apply_url, retired_at
ON public.jobs
