...
def evidence_missing_prompt(evidence: dict[str, Any]) -> str | None:
    item = normalize_evidence(evidence)
    skill = item["skill"]
    if not item["proof"]:
        return f"What proves the {skill} milestone happened?"
    if not item["impact"]:
        return f"Did the {skill} milestone reduce time, unblock a project, improve quality, or ship something?"
    return None
...
def build_cv_draft(
    ...
    lines: list[str] = [
        ...
        "",
        "Evidence-led update",
        "This draft preserves your uploaded baseline CV and adds only progress evidence you logged in Truth Mirror.",
    ]
    if score_delta is not None:
        sign = "+" if score_delta >= 0 else ""
        lines.append(f"Truth Score movement since last CV: {sign}{score_delta:.1f}")
    lines.extend(["", "New verified progress"])

    for item in normalized:
        parts = [
            f"- {item['date']}: {item['skill']}",
        ]
        if item["task"]:
