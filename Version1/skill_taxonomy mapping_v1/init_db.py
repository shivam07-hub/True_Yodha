...
def find_latest_full_csv(company_folder: Path) -> Path | None:
    """Return the most recent jobs.csv in the company's Outputs/{date}/ subfolder."""
    outputs = company_folder / "Outputs"
    if not outputs.is_dir():
        return None
    date_dirs = sorted(d for d in outputs.iterdir() if d.is_dir())
    for date_dir in reversed(date_dirs):
        csv_path = date_dir / "jobs.csv"
        if csv_path.exists():
            return csv_path
    return None
...
def discover_companies(csv_base: Path, filter_names: list[str] | None = None) -> list[tuple[str, Path]]:
    ...
    for company_dir in sorted(csv_base.iterdir()):
        if not company_dir.is_dir() or company_dir.name in ("Manual_Tag", "Outputs"):
            continue
