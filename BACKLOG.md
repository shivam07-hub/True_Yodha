...
43. **Myrology landing rebuilt on source-labelled figures — ✅ SHIPPED `f9aa5a5e` Develop 2026-08-26 (artboards 1a/1b/1c of 4).** Built from the claude.ai/design handoff *Myrology UI mockups and redesign* (project `11c08d2d-…`); local copy `reference/design_handoff_myrology_landing/` (⚠️ `reference/` is GITIGNORED — local-only, like every other handoff). **The defect it closed:** the page sold a paid reading while printing a chart nobody had cast — an `ASTRO_CELLS` grid ("SUN SIGN Libra 15° 24′", "DASHA Jupiter 5y 7m left") and a radar on hardcoded `VALS`, shown as the visitor's own before a birth date existed. **The rule now:** every figure is either cast from what the visitor typed or counted from the live index, labelled by source and never averaged — amethyst = chart lens, teal = myro live data, gold = astrologer. This is the two-lens guardrail (INTEGRATOR ITEMS, 2026-05-31) finally rendered rather than merely agreed. Teal was promoted to a FOREGROUND accent inside `.myrology-root` only, reversing that file's own header rule — the source system collapses without it; the core product accent is untouched. **Backend (additive):** `/public/stats` gained `industry_groups` (top 5), `total_industries`, `role_families` — all already computed by `compile_market_analytics()` behind the existing 1h `shared_cache`, so this costs one count query per hour and nothing per pageview. New `GET /myrology/order` derives the delivery promise from `billing_payments.verified_at` — the canonical money record, NOT a new profile column and NOT `intake.updated_at` (which a later edit would reset). **Design's numbers were placeholders; the real ones are 10 canonical industry groups (not 11) and 334 `role_family_labels` (not 64).** Gates: 2385 backend · 764 frontend · tsc/lint/ui-drift/build clean; verified at 1280 and 375 against a local backend. Memory: `project_myrology_two_lens_landing`.

    **OPEN — needs Shivam:** (1) **Artboard 1d, the delivered Career Map** (`/myrology/map/:id`) — the only genuinely new machinery: a table for delivered maps plus an astrologer-side write path. ⚠️ It was wrongly blocked first on "not a live report engine"; that line bans Myro COMPUTING a reading, not rendering one a human wrote (INTEGRATOR ITEMS now carries the correction). (2) **Any new tier.** The handoff proposed ₹1,499 / ₹3,999 "Go Deeper" — each is a new SKU needing a `PRODUCTS` entry, entitlement key, refund copy, and a human to deliver it, against one astrologer. (3) **The inclusions question.** The handoff downgrades ₹299 from "3 lifetime sessions" to "one 30-min session + a market refresh"; Shivam said keep 3 sessions, so ₹299 and its inclusions are untouched — but marketing copy and the paid `BookingPanel` must move together or the page advertises one thing while checkout promises another.

    **Fixed in passing:** the FAQ promised a partial post-delivery refund that Terms §07 has never offered (full refund before delivery, non-refundable after). Now matches. Also the wheel's house labels sat 16px from their glyphs and ran through them ("PUBLI♇", "☉RAVEL") — labels now anchor outward on the flanks.

    **⚠️ The CSS split nearly shipped a 5-column strip to a 375px phone,** and all five gates were green while it did. Splitting `myrology.css` three ways left the responsive overrides in an earlier file than the grids they collapse; a media query adds NO specificity, so the base rule in the later-imported sheet won. Rule: responsive overrides live beside the rules they override, below them. Only looking at the page caught it.
