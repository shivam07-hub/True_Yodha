...
| **SURF1** | **Three surfaces, not two: product-dark, paper-light, and marketing INK.** Ink is a cool navy — page `#050a18`, card `#0b1424`, elevated `#0f1a2e` — and it is **dark-only**. It owns the public marketing pages; the product keeps warm-dark (`#191918`) and paper-light unchanged. Opt a route in with `tm-ink` on its top-level wrapper; `app/design-tokens.css` §Surface: INK does the rest, and `tests/brand-system.test.ts` locks the hexes and the selector shape. **Why a third surface instead of restyling components:** the accent `#00f5d4` is cool (hue 173°), so on the warm base it was the only cool thing on the page and read as a sticker rather than light off the surface — measured blue-bias −1.4 warm vs +20.2 ink. Cards also gained a real edge (2.2× page luminance, against 1.6× where card and page were nearly the same colour). Applied 2026-08-17 to `/` only; extending it to the other public routes (`/intel`, legal, docs) is still open — see BACKLOG. |
| **SURF2** | **Ink outranks light by SPECIFICITY, never by source order.** The selector is `html:root:has(.tm-ink)` = (0,2,1), which beats `:root[data-surface="light"]` = (0,2,0) wherever it sits in the file, and it re-pins the teal interactive ramp plus `body::before` so a light-preference visitor still gets a correct dark marketing page. Re-scoping it to a bare `:root:has(…)` ties on specificity and silently breaks **only for visitors who chose light** — which is why the shape is asserted in a test rather than left to review. |
