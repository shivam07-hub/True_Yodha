...
| **SURF1** | **Three surfaces, not two: product-dark, paper-light, and marketing INK.** Ink is a cool navy — page `#050a18`, card `#0b1424`, elevated `#0f1a2e` — and it is **dark-only**. It owns the public marketing pages; the product runs azure ash (`#161a1c`) and cool paper (`#f3f7f9`). Opt a route in with `tm-ink` on its top-level wrapper; `app/design-tokens.css` §Surface: INK does the rest, and `tests/brand-system.test.ts` locks the hexes and the selector shape. **Why a third surface instead of restyling components — one of the two reasons has since died.** ⚠️ The original case was (a) the accent is cool and the product base was warm, so the accent read as a sticker rather than light off the surface (blue-bias −1.4 warm vs +20.2 ink), and (b) cards gained a real edge (2.2× page luminance against 1.6×). **ACC1 (2026-08-23) killed (a)** — the product base now measures +4.0, so the accent is no longer the only cool thing on the page. **(b) is unchanged and still true** (1.54× on the azure ramp). Ink is kept on the surviving reason, but it is now a thinner case than it was approved on and is a live candidate for retirement — Shivam's call, not a cleanup. Applied 2026-08-17 to `/` only; extending it to the other public routes (`/intel`, legal, docs) is still open — see BACKLOG. |
| **ACC1** | **AZURE supersedes teal as the interactive accent, on every surface (2026-08-23, Shivam).** `--tm-interactive` is `#4fc7f6` dark / `#0072be` light / `#4fc7f6` ink. Neutrals moved with it: every ramp rung now sits at ~228° with chroma 0.007→0.016 — the accent with its saturation almost off — at the **same lightness as before**, so no surface got darker or lighter. Two dead rungs were fixed in the same pass (`surface`→`surface-2` stepped 1.0 L*, `surface-3`→`border` 1.4 L*; both now ~3). **Two reasons, and only one of them is taste.** (1) Chroma: `#00f5d4` is 0.160, which is the *neon* tell (ANTI_SLOP §29, formerly LOCKED); azure is 0.125. Swapping to a different neon would have changed nothing. (2) Hue collision: teal at 178° sits **33° from `--tm-success` (145°)**, so a coloured verdict and a control were near-neighbours — which is why the market card needs a green tick. Azure clears every status band (success 145°, warning 75°, danger 25°) by **83°**. **The MARK stays teal.** `--tm-brand` `#00f5d4`, the OG image, the particle fields and the skill graph are untouched — that is the whole point of the brand/interactive split, and `tests/brand-system.test.ts` now asserts it so nobody "fixes the inconsistency". **Light stopped being its own brand.** Warm Firecrawl paper + orange retired; light is cool paper `#f3f7f9` + `#0072be`. Note the light accent is hue **248°**, not 228° — at paper lightness sRGB cannot hold chroma at 228° and the colour desaturates toward cyan, i.e. back toward the retired teal. **`--tm-info` lost its hue** (was `#8BA3FF` / `#2F4FBF`): blue is the accent now, so a blue info tier reads as a dim control. It is neutral on every surface and keeps its wash+border shape. ⚠️ **Forge (amber, `data-accent="forge"`) is now the only accent in the file that does not clear the status ramp** — ~5° from warning. True before this change too, so not a regression, but fix it or retire it. |
| **SURF2** | **Ink outranks light by SPECIFICITY, never by source order.** The selector is `html:root:has(.tm-ink)` = (0,2,1), which beats `:root[data-surface="light"]` = (0,2,0) wherever it sits in the file, and it re-pins the azure interactive ramp plus `body::before` so a light-preference visitor still gets a correct dark marketing page. Re-scoping it to a bare `:root:has(…)` ties on specificity and silently breaks **only for visitors who chose light** — which is why the shape is asserted in a test rather than left to review. |
