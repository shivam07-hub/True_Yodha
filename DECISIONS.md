...
| XP1 | **Myro Coins are permanent** — never reset. Wallet the user owns forever. (Renamed from "XP" end-to-end 2026-06-22, closed, see [ARCHIVE.md](ARCHIVE.md) 2026-09-15 — DB, RPCs, wire and UI all say coins. `XP*` row IDs kept only because other docs cite them.) |
...
| MTR1 | **The billable unit for the partner API is a monthly ACTIVE seat** — a seat that signed in during the month, never the roster size. So the recorded fact is a session and the count is `count(distinct subject_id)`; a counter column cannot answer it. Locked 2026-09-05. |
| MTR2 | **The meter records, it never blocks.** No plan, no quota, no enforcement column, and the write is a background task after the response leaves. Partner SSO is the sign-in path for 281 real students. A lost write undercounts, which is the acceptable direction for a distinct-seats unit. |
| MTR3 | **`partners` is the account table for every B2B product.** No second tenant table: a dataset customer is a row in `partners` writing a different metric to `partner_usage_events`. Metric names are dotted and CHECK-enforced (`sso.session`), so a typo is a write error, not a silent second metric. |
| MTR4 | **Billing months are IST**, via a fixed `+05:30` interval. A 4am IST login on the 1st is 22:30 UTC on the previous month's last day; UTC would bill it to the wrong month. Stored as a `date` because every variant of `to_char` is STABLE and Postgres refuses it in a generated column. |
| MTR5 | **Both SSO modes are metered**, with the mode in `detail`. Whether `connect_required` (a consent screen the user may not finish) counts as an active seat is a pricing question left open on purpose — recording both keeps it answerable without re-instrumenting. |
| MTR6 | **Per-request volume is deliberately NOT metered.** The key ceiling is 600/min, so a row per request is unbounded by design and nothing bills on it. `partner_usage_events` accepts that metric later with no schema change. |
...
| **SURF1** | ~~Three surfaces, not two.~~ **RETIRED 2026-08-23 (ACC1).** Marketing INK is deleted; the public pages run the same two surfaces as the product and follow the visitor's theme. Both measurements it was approved on had gone void. (a) "The accent is cool, the product base is warm, so the accent reads as a sticker" — the azure base measures +4.0 blue-bias against the old −1.4, so the accent is no longer the only cool thing on the page. (b) "Cards earn their edges here — 2.22× page luminance against 1.6×" — **that metric was wrong for the job.** A luminance RATIO divides by ink's near-black page (Y=0.0035) and inflates. In L*, the units the eye actually reads, ink's card sits **+3.5** over its page and the azure ramp's sits **+3.9**. Ink had LESS edge, not more. The lesson is the metric, not the surface: at near-black, ratios flatter. Measure separation in L*. |
| **ACC2** | **The second accent has no hue: Ember (amber) becomes BONE, an achromatic ivory (2026-08-23).** `data-accent="forge"` now yields `--tm-interactive: #f1e7d2` with `#191712` ink — 14.27:1 on the page. **Why amber had to go:** it sat ~70°, five degrees from `--tm-warning`, so a verdict chip and a control were the same colour. That was true on the teal palette too and was carried as a known cost — but azure clears every band by 83°, which left amber as the only accent in the file that did not, and a known cost nobody can see is just a defect. **Why not simply another hue:** the safe arc is 205–250° (purple banned, everything warm collides) and azure already sits at 228°, so any compliant hue lands within ~20° of the default and does not read as a choice. Removing the hue is the only move that is both safe and visibly different. **What it costs:** an ivory link is the same lightness as body text (ΔL* 3.5), so colour alone cannot signal clickable. Bone rests on the other three signals — cursor, hover, focus ring — which still satisfies the Four-Signal rule and matches how every non-accent control already works under `--tm-interactive-rest`. ⚠️ The weak spot is an inline link in prose, and the `.tm-link` underline Bone pins does NOT close it: **that primitive has 6 usages and zero component consumers while 219 CSS rules hand-roll accent colour.** Adopting `.tm-link` is the real fix and is its own job. **The stored key stays `"forge"`** — it is the `accent_pref` / `data-accent` identifier and has outlived two renames (Forge → Ember → Bone). Changing it would strand every existing preference row. Users already on amber move to Bone without acting, deliberately. |
| **ACC1** | **AZURE supersedes teal as the interactive accent, on every surface (2026-08-23, Shivam).** `--tm-interactive` is `#4fc7f6` dark / `#0072be` light. (Marketing INK also carried it for a few hours before SURF1 was retired the same day.) Neutrals moved with it: every ramp rung now sits at ~228° with chroma 0.007→0.016 — the accent with its saturation almost off — at the **same lightness as before**, so no surface got darker or lighter. Two dead rungs were fixed in the same pass (`surface`→`surface-2` stepped 1.0 L*, `surface-3`→`border` 1.4 L*; both now ~3). **Two reasons, and only one of them is taste.** (1) Chroma: `#00f5d4` is 0.160, which is the *neon* tell (ANTI_SLOP §29, formerly LOCKED); azure is 0.125. Swapping to a different neon would have changed nothing. (2) Hue collision: teal at 178° sits **33° from `--tm-success` (145°)**, so a coloured verdict and a control were near-neighbours — which is why the market card needs a green tick. Azure clears every status band (success 145°, warning 75°, danger 25°) by **83°**. **The MARK stays teal.** `--tm-brand` `#00f5d4`, the OG image, the particle fields and the skill graph are untouched — that is the whole point of the brand/interactive split, and `tests/brand-system.test.ts` now asserts it so nobody "fixes the inconsistency". **Light stopped being its own brand.** Warm Firecrawl paper + orange retired; light is cool paper `#f3f7f9` + `#0072be`. Note the light accent is hue **248°**, not 228° — at paper lightness sRGB cannot hold chroma at 228° and the colour desaturates toward cyan, i.e. back toward the retired teal. **`--tm-info` lost its hue** (was `#8BA3FF` / `#2F4FBF`): blue is the accent now, so a blue info tier reads as a dim control. It is neutral on every surface and keeps its wash+border shape. **Closed same day — see ACC2.** |
| **SURF2** | ~~Ink outranks light by SPECIFICITY.~~ **RETIRED 2026-08-23 with SURF1.** Kept here because the trap is general: `html:root:has(.x)` = (0,2,1) beats `:root[data-surface="light"]` = (0,2,0) wherever it sits, and a bare `:root:has(…)` ties instead — which breaks **only for visitors who chose light**. If a scoped surface is ever reintroduced, assert the selector shape in a test, not in review. |
| **FIELD1** | **Both surfaces have a page field (2026-08-30, Shivam).** Dark keeps the starfield. Light is dawn: a warm bloom parked antipodal to the pointer on an 8×8 of the viewport (about 6–7 king-moves), cool water on the work side, and a coloured cast that falls from the sun. Light is still not its own brand — same hue-228 paper, same azure accent. The field is depth, not a second interactive colour. `LightFieldDriver` is the single writer of `--tm-sun-*` / `--tm-cast-*`; math is `lib/theme/antipodal-sun.ts`. Reduced motion and coarse pointers keep the parked rest. Chrome frosts over the field on both surfaces. Myrology still owns its own sky. Spent the ANTI_SLOP slot that neon left (item 31). |
