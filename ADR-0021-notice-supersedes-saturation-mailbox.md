...
We retired it. A **Notice** (CONTEXT.md) is the record — `cause_key`, not route. Failures 1–7 open a row live (no email). A daily GitHub Action is the closer; one digest per run informs. Slow 200s are keyed by kind (`reads_over_budget` vs `capacity_queue`), never by path.

We accepted a 23-hour gap with no pager in exchange for one operator truth. Live detection without live paging is deliberate: the row exists for the closer; the mailbox does not come back “for safety.” Skill-floor Resend is the same mailbox and died with it.
